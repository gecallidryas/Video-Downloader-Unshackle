...
| 2 | `captureCredentialHeaders` defaults to `true`; stores Cookie/Authorization | done | Policy risk | Release default is safe: credential header capture defaults to `false`. Passive manifest hydration (`fetchManifest`) now sends `credentials: 'include'` only when `advancedMode && captureCredentialHeaders`, otherwise `omit` — no cookie leak during detection by default. |
| 3 | Request header context forwards sensitive headers | improved | live-stream (referer/origin only), stream-detector, cat-catch | Header context forwards safe Referer/Origin to native FFmpeg asset calls; Cookie/Authorization stay out of UI/log/diagnostics by default and require advanced explicit credential capture. All engine handoff (thumbnail/preview/native export) now flows through the single `buildEngineHandoff` policy path, which emits Cookie/Authorization only when `advancedMode && captureCredentialHeaders` — the ad-hoc raw-header read was removed. Captured by-URL header context is now retained with a TTL (request-id state deleted separately) so the export engine can still read it after the request completes. |
| 4 | Production HLS/DASH path audit — confirm native export is intended default | improved | Unified | Native export remains optional and setting-gated; effective native availability is now resolved from the setting AND `nativeMessaging` permission AND a successful host PING (`resolveEffectiveNativeFeatures` / `native-feature-gate.ts`) instead of a hardcoded `false`, and long-running native export uses a persistent `connectNative` Port so streamed PROGRESS messages reach the UI. Browser-only HLS now resolves an explicit route before segment fetch, probes first media-segment bytes for TS/fMP4 and PAT/PMT codec evidence before mux.js, and only routes to mux when a successful TS probe proves it OR the source is AES-128 with declared H.264/AAC codecs (decrypted upstream before transmux). The browser DASH fallback now refuses multi-track plans with an honest "enable native FFmpeg" diagnostic instead of concatenating audio+video into an undemuxable artifact. mux.js stays out of the MV3 background bundle. Native export now reads the COMPLETED host-private output back over a chunked reader (`native-asset-server.readFullOutput`, 512 KiB raw chunks to stay under the native-messaging ~1MB framing limit), delivers the assembled blob through `chrome.downloads`, and forwards native PROGRESS to the job store; if chunked output reads are not configured it fails loudly instead of returning a phantom local-path output. End-to-end wiring is now complete: `READ_ASSET_BYTES`/`NativeFfmpegReadAssetBytesPayload` carry an optional `offset`, the helper dispatcher slices `[offset, offset+maxBytes)` and returns an `eof` flag (ranged reads bypass the whole-file `ASSET_TOO_LARGE` cap), `background.ts` constructs the asset server with a `readOutputChunk` bound to the client and passes `readFullOutput` into the `nativeExport` runner call. Native EXPORT now receives the gated `buildEngineHandoff` headers in its payload (previously only thumbnail/preview did), so Referer/Origin/Cookie-gated hosts no longer 403/401. Helper ranged reads now open a single file descriptor and read only the requested window (was an O(n²) whole-file `readFile` per chunk), the export job threads `expectedDurationSec` from a duration probe so progress is live (was stuck at 0% until completion), the helper enum-validates `protocol`/`outputKind`, includes the originating `requestId` in fatal ERROR envelopes, and redacts credential header values from error output. Extension-side delivery streams chunks to an OPFS-backed file and loops on the authoritative `eof` flag instead of accumulating the whole output as one in-heap Blob / mis-detecting short reads as EOF. Browser DASH export now applies a memory ceiling (refusing oversize single-track DASH unless writing direct-to-disk) and finalizes confident single-track fMP4 as a correctly-labeled `.mp4` instead of a mislabeled `.m4s`. The install/registry/extension-ID story is now resolved without a hardcoded ID: the extension reads its live `chrome.runtime.id` and detects the host browser (`native-host-browser.ts` / `detectNativeHostBrowser`), then generates a self-contained double-click `.bat` (`generate-installer-bat.ts`) whose embedded PowerShell bakes that exact id into `allowed_origins` and writes the native-messaging manifest to the correct per-browser HKCU hive (Chrome/Edge/Brave/Chromium). Installer PowerShell now requires `-ExtensionId` (no `gljda…` fallback) and takes `-Browser`; both popup and side-panel surfaces deliver the generated installer via `native-installer-download.ts`. This survives the unknown Chrome Web Store id. |
...
| 6 | Protected-content refusal checks for deep capture modes | improved | cat-catch (has it, risky) | Protected/DRM/SAMPLE-AES media is refused for native/browser export and asset generation; unclassified `unknown` protection no longer blocks thumbnail/hover asset generation unless the candidate is actually protected. `DRM_DETECTED` runtime messages now have a real effect: the router records the detection in a live map AND marks matching candidates as `protection.kind: 'drm'` / `status: 'protected'` so EME-detected streams are refused downstream (the handler no longer returns a phantom `{ ok: true }` when no detection map is wired). |
...
| # | Item | Status | Stronger in | Action |
|---|---|---|---|---|
| 10 | Broken-pipe recovery and ranged resume | done | live-stream (strong) | Scheduler retries partial fetches with validated retained bytes, resumes ranges only when safe, rejoins bytes, and lowers effective host concurrency after repeated recoverable failures. `run-hls-job` now passes an IndexedDB fragment store to the scheduler by default, so segment-level resume actually engages across restarts (previously wired but never activated). On resume the scheduler now READS previously-stored fragments back through the same `onSegmentComplete`/export sink in index order (via a new `readFragment` storage method) instead of skipping them — fixing silent truncation/corruption where a resumed download was missing exactly the fragments it had already saved. The streaming path no longer also retains the full file in a `results[]` array, so multi-hundred-MB downloads stay bounded in memory; an in-memory-finalizing route still enforces the size ceiling. |
...
| 13 | Timeline/discontinuity handling | done | live-stream (user timeline choice), Unified | `discontinuityPolicy` is now part of `DownloadSelection`, read by `runHlsJob`→`planHlsSegments`, set via the `SET_HLS_DISCONTINUITY_POLICY` runtime message + client method, and toggled from the queue overflow menu ("Skip ad discontinuities" / "Include all segments"), which re-plans and requeues the job. Covered by run-hls-job + router tests. |
...
| 16 | Fetch retry backoff policy | done | puemos (100ms × 1.15x) | Extracted `computeBackoffDelay` with cap/jitter tests and scheduler coverage. Now parses the server `Retry-After` header (delta-seconds and HTTP-date) and honors it up to a 60s ceiling; default fragment attempts raised from 3 to 5. |
...
| 20 | Live HLS retry telemetry | done | cat-catch, live-stream | `updateHlsSegmentProgress` now persists `event.liveHlsTelemetry` onto the job (`DownloadJob.liveHlsTelemetry`), SidePanelApp maps it into the queue item, and `QueueItem` renders a live/idle telemetry line (state, last sequence, refreshes, idle retries). |
...
| 31 | DASH live/SegmentTimeline robustness | done | Unified | DASH parser/inspector cover dynamic MPDs and `SegmentTimeline` expansion with tests. Browser-only templating now also substitutes `$Number$` in SegmentTimeline media templates, `$Bandwidth$` (init+media, with `%0Nd` padding), and the `$$`→`$` escape via shared `dash-template.ts` (previously these emitted literal tokens → 404). |
...
| 35 | HLS segment repair controls | done | cat-catch | `selectSegmentsForRepair()` is now invoked by the `REPAIR_HLS_SEGMENTS` runtime message (index-range/time-range/regex selectors) which marks matching segments pending and requeues. `JobSegmentStatus` carries `durationSec` so time-range selection works; queue overflow menu exposes "Repair by URL pattern…" and "Repair by time range…", wired through the client. Covered by router + client + QueueItem tests. |
| 36 | HLS range expansion tests | done | cat-catch | `expandSegmentRangeTemplate()` is now wired into `createManualHlsIngestEvidence`: a single-line `${range:start-end,pad}` URL expands into a synthetic segment manifest (mode `range-template`), checked before the raw-list fallback. Covered by manual-hls-ingest test. |
...
| 58 | Bucket metadata persisted separately | done | puemos | Scheduler `onFragmentStored` now calls `bucketMetadataStore.recordChunk()` on every fresh fragment write (threaded background → runBrowserHlsExportJob → runHlsJob → scheduleSegments); metadata persists to chrome.storage.local via `createChromeBucketMetadataPersistence`. Browser DASH export buffers single-track in memory and writes no fragment bucket, so it has no per-bucket metadata to record (accurately contributes 0 to bucket usage). |
| 59 | Track bytes written and stored chunks | done | puemos | Production HLS segment writes record bytes/chunk counts per bucket; covered by scheduler `onFragmentStored` test (resumed fragments excluded from double-counting). |
...
| 61 | Rehydrate bucket from metadata after worker wakeup | done | puemos | `bucketMetadataStore.rehydrate()` runs in the background `rehydrateState()` path, loading persisted per-bucket metadata from chrome.storage.local on SW wakeup; covered by rehydrate test. |
| 62 | Measure bucket usage if metadata missing | done | puemos | Background `GET_STORAGE_DIAGNOSTICS` aggregates per-bucket bytes via `estimateBucketUsage()`, falling back to measuring stored fragments when a bucket has no metadata; surfaced in StorageFooter + popup summary. |
| 63 | Separate subtitles IndexedDB | done | puemos | Background now wires `createIndexedDbSubtitleStore()` (IndexedDB-backed with memory fallback) so pre-stored subtitles survive SW restart. The pre-stored content is also delivered to the user as sidecar downloads on the ffmpeg success path (#115). |
| 64 | Estimate subtitle byte usage | done | puemos | Subtitle byte totals flow into `GET_STORAGE_DIAGNOSTICS` (`subtitleBytes`) and render in the StorageFooter breakdown and popup Storage summary. |
...
| 66 | Near-quota warning (90% or <=200MB free) | done | puemos | Side-panel now consumes the canonical `getStorageDiagnostics`/`storageQuotaLevel` (90%/<=200MB) via `GET_STORAGE_DIAGNOSTICS`; the local fallback `computeStorageLevel` delegates to `storageQuotaLevel`, ending the prior 95/80/60 drift. |
| 67 | Low storage banner component | done | puemos | `StorageFooter` renders diagnostics `warning` copy (role="alert") plus a fragments/subtitles breakdown; popup Storage summary shows the same warning. |
| 68 | Storage summary in Settings and Downloads footer | done | puemos | Popup "Storage & Output" now shows a diagnostics-backed usage summary (used/quota, fragment+subtitle breakdown, warning) refreshed after cleanup; Downloads footer uses the same source. |
| 69 | Auto delete after save setting | done | puemos | Added `autoDeleteAfterSave` setting, popup control, controller cleanup hook, fragment/metadata/subtitle cleanup, and a settings maintenance action for orphaned browser fragment buckets plus cached detection thumbnails. |
...
| 71 | Playable HLS export output | improved | hls_downloader (`-f` flag), cat-catch | Browser HLS export no longer saves raw `.ts`, staged `.m4s`, or octet-stream artifacts. It only succeeds with mux.js-created MP4; unsafe codec hints, unknown containers, fMP4 assembly, disabled mux.js, or failed transmux now require native FFmpeg instead of downloading segment files. Missing codec hints can proceed when the first media segment proves mux.js-safe MPEG-TS/H.264/AAC or MP3 via PAT/PMT. Offscreen export starts only when the first segment is ready, stays warm during active exports, appends segments in strict plan order, and uses a single persistent mux.js transmuxer (replacing the prior per-segment recreation that produced negative/overflowed `tfdt` values and ~13h inflated duration); finalize validates MP4 structure before reporting success. |
...
| 85 | Progressive preview while downloading | ~~partial-core~~ done | live-stream (MP4Box/MSE), Unified | `deriveDownloadedRanges` (`src/core/preview/derive-downloaded-ranges.ts`) maps the live job's `segmentStatuses` (cumulative per-segment durations) to contiguous `done` ranges; `SidePanelApp` passes `downloadedRanges`/`totalDurationSec` into `PreviewModal`, which renders the gradient progress bar. Honest limitation: no MSE source is wired, so `liveSegmentSource` is intentionally omitted rather than faked. |
| 86 | Codec sniff via MP4Box | ~~partial-core~~ done | live-stream, Unified | New `GET_CODEC_INFO` runtime round-trip: `runtime-router.ts` `resolveCodecInfo` reads the first downloaded fragment from the background `fragmentStore` and runs `sniffCodecs`; `SidePanelApp` fetches it once per previewed candidate and feeds `PreviewModal.codecInfo`, with `isCodecSupported` driving the `CodecBadge` unsupported warning. Limitation: badge appears only after ≥1 segment is downloaded (byte-accurate, no manifest guessing). |
| 87 | hls.js preview when native HLS unsupported | ~~improved~~ done | puemos | `usePreviewPlayer` now reports the active `playbackStrategy` (`native` vs `hls-js`) and `PreviewModal` surfaces it as an observable indicator (`data-testid="preview-playback-strategy"`), so the hls.js fallback path is no longer internal-only. |
...
| 89 | Preview duration callback | ~~partial-core~~ done | puemos | `PreviewModal.onDurationResolved` → `SidePanelApp` → new `SET_CANDIDATE_DURATION` runtime message → `candidateRegistry.setDuration` persists `durationSec` onto the candidate (survives panel refresh via `GET_CANDIDATES`). |
...
| 102 | Current/all/previous candidate views | improved | stream-detector | Current Tab now follows browser tab activation while the side panel stays open; All Tabs hydrates passive request-journal detections across tab buckets before listing; Previous Session remains capped by `previousSessionLimit` and strips bulky non-WebP data thumbnails. |
...
| 115 | Subtitle mux to MKV with WebVTT verification | partial-core | puemos | `verifySubtitleTrack` remains a tested helper; native post-mux ffprobe verification is still not wired. Sidecar fallback IS now wired on the ffmpeg path: `runNativeExportJob` delivers fetched subtitle content as sidecar downloads on success (sidecar/both, and embed as a fallback since exportMedia carries no subtitle URLs to truly embed). |
...
| 140 | mux.js as lightweight browser-only TS-to-MP4 fallback | improved | m3u8-downloader, cat-catch | `mux.js` now loads only in the offscreen export host, receives MPEG-TS segments from the background-safe export protocol after byte-level TS/PAT/PMT probing, allows missing manifest codec hints when segment bytes prove mux.js-compatible streams, starts lazily at first segment append, keeps the offscreen session alive while exporting, uses one persistent mux.js transmuxer reused across all segments (mux.js owns timestamp continuity, fixing the prior per-segment-recreation bug that wrote negative/overflowed `tfdt` values and inflated player duration to ~13h), validates produced MP4 structure (ftyp+moov + `tfdt` below the 2^32 wrap) before claiming success, transfers raw segment bytes via structured clone instead of base64, dedupes repeated init boxes, reports typed mux diagnostics, dropped the parallel raw-recovery sink, serializes concurrent segment completions in plan order, and has a build regression preventing background mux/preload imports. |
