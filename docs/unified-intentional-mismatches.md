...
| Source video cards are built from imperative DOM templates. | Target card behavior is split across `MediaCard`, `VariantPicker`, and `TrackPicker`; trim selection lives in `PreviewModal` through `TrimSlider`. | Keep the React component model testable and typed while keeping cards compact. |
