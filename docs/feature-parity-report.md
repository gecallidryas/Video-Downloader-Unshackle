...
| P3 | Consider mux.js as lightweight browser-only TS-to-MP4 fallback | `src/core/export/muxjs-transmuxer.ts`, `src/offscreen/export-host.ts`, settings/capabilities | Done: `mux.js` is loaded only in offscreen, MPEG-TS segments transfer through a JSON-safe command protocol after TS/PAT/PMT probing, MP4 output flushes once per job and dedupes repeated init boxes, failures carry route/sink/segment/mux-code diagnostics without saving raw segment downloads, and the MV3 background bundle is tested against mux/preload imports. |
