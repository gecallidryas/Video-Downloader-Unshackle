...
| Request header context | improved | present, broad | present, referer/origin DNR only | Safe Referer/Origin context is forwarded to native asset FFmpeg calls; Cookie/Authorization are excluded from UI/log/diagnostics and remain advanced-mode explicit opt-in for capture. |
