...
| Segmented download engine | Present in core tests; production path prefers native FFmpeg when enabled and now has a first-class browser-only HLS route that streams decrypted raw/recovery segments to offscreen export sinks while downloading; DASH remains bounded/native-first. | Present | Strong custom `MyGet` range/thread engine | Keep scheduler/core policy in background, but keep DOM/media export work offscreen. |
...
| Output conversion | Native FFmpeg helper, MP4/WebM/audio/trim paths, offscreen browser HLS TS-to-MP4 via mux.js, OPFS/File System Access/browser memory sinks for MP4 output, and direct WebM trim recording. | FFmpeg/offscreen baseline | Downloads raw stream/media; no FFmpeg mux UI | Native helper remains optional; browser-only HLS no longer imports mux.js in the MV3 background, requires byte-level TS/PAT/PMT codec evidence before mux.js, allows missing manifest codec hints when the segment probe proves mux.js-compatible streams, refuses raw/staged HLS artifacts, lazily starts and keeps alive offscreen HLS MP4 sessions, serializes segment appends in plan order, rebases per-segment fMP4 decode timestamps to prevent inflated player duration, and reports route/sink/segment/mux-code diagnostics on browser mux failure. |
| Preview | Background-owned poster and hover assets are queued/deduped separately from full preview playback; generated posters use compact WebP thumbnails, hover clips stay runtime-only instead of persisted asset-cache entries, the eye button opens the original source/manifest, native extraction is preferred for HLS/DASH and large/unknown direct media, and advanced-mode diagnostics stay sanitized. | Preview/thumbnail baseline | MSE/MP4Box preview while downloading | Live-stream's progressive preview concept remains useful but optional; validate the new asset pipeline on popular sites before claiming stronger parity. |
...
| Request header context | improved | present, broad | present, referer/origin DNR only | Safe Referer/Origin context is forwarded to native asset FFmpeg calls; Cookie/Authorization are excluded from UI/log/diagnostics and remain advanced-mode explicit opt-in for capture. All thumbnail/preview/native-export handoff now flows through the single gated `buildEngineHandoff` builder (Cookie/Authorization only when `advancedMode && captureCredentialHeaders`), native export finally receives those headers, and captured by-URL context is retained with a TTL so the engine can read it after request completion. |
...
| HLS discontinuity/timeline handling | present | present | present with user timeline choice | Planner groups discontinuity timelines and supports include-all/skip-ads policy; user choice is now wired end-to-end via `DownloadSelection.discontinuityPolicy`, the `SET_HLS_DISCONTINUITY_POLICY` message, and a queue overflow-menu toggle that re-plans/requeues. |
...
| DASH live/SegmentTimeline robustness | present | present | parser library dependent | Parser and inspector now cover dynamic MPDs and `SegmentTimeline` expansion with tests. Browser-only segment templating substitutes `$Number$`/`$Time$` in timeline templates, `$Bandwidth$` (with `%0Nd` padding), and the `$$` escape. Known remaining browser-path gaps: separate audio+video adaptation sets are refused (need native FFmpeg to mux), and `SegmentBase`/`indexRange`/sidx is not parsed. |
...
| File System Access direct writes | present | not primary | present | Browser HLS exports can stream through offscreen File System Access when a persisted output folder is available, with OPFS staging and bounded memory fallback selected before segment fetch. |
...
| Broken-pipe recovery and ranged resume | present | present | strong | Scheduler now resumes partial fetches only with validated retained bytes, rejoins recovered data, and lowers effective host concurrency after repeated recoverable failures. On restart it also reads previously-stored fragments back through the export sink in index order (new `readFragment`), instead of skipping them and silently truncating the resumed output, and the streaming path no longer doubly retains the full file in memory. |
| User URL replacement on failed segment | partial-core | not clear | present | Helper exists, but user recovery workflow is deferred for expired live URLs. |
| Segment concurrency | present | present | present | Keep settings-driven scheduler; live-stream caps UI threads to 1-5. |
| Per-host concurrency/bandwidth | present | present | gap | Unshackle/Unified are stronger. The user-configured `maxBandwidthPerHostKBps` cap now actually reaches the segment scheduler/bandwidth limiter at runtime (controller threads it through as bytes/sec); previously the setting was defined but never wired and silently did nothing. The direct-media range download path now forwards the same cap (`downloadDirectWithRanges` → `scheduleSegments`), so throttling is no longer a no-op for direct downloads. |
| Batch timeline/download jobs | partial-core | present | present via directory picker | Helper exists; production batch directory/job creation is deferred. |
| Persistent output directory handle | improved | gap/unknown | present | Output-folder persistence is opt-in through `rememberOutputFolder`; popup selection now persists handles for the background direct-to-disk path. |
...
| Progressive preview while downloading | present (`deriveDownloadedRanges` → `PreviewModal` range bar) | present | present with MP4Box/MSE | `SidePanelApp` derives `done`-segment ranges from the live job's `segmentStatuses` and passes `downloadedRanges`/`totalDurationSec`; MSE/live-segment streaming is honestly out of scope (no source wired), so `liveSegmentSource` is omitted rather than faked. |
| Codec sniff via MP4Box | present (`GET_CODEC_INFO` round-trip + `CodecBadge`) | present | present | Background `resolveCodecInfo` sniffs the first downloaded fragment via `sniffCodecs`; `SidePanelApp` feeds `PreviewModal.codecInfo` and `isCodecSupported` flags unsupported codecs. Byte-accurate (post first segment) vs MP4Box init-segment parsing. |
...
| Side panel UX | present with live current-tab refresh and active browser-tab switching | present | gap | Keep Unshackle UX. |
| Popup job details | partial | present | present | Consider a job-details modal/panel for advanced diagnostics. |
| Native FFmpeg export | present with optional popup permission flow, host/dependency diagnostics, beta/dev PowerShell setup, Windows `.exe` native host launcher, operation-specific native timeouts, and a settings/onboarding switch to disable all native paths; background native client creation is lazy and skipped while native features are disabled | present in source baseline as offscreen/wasm idea; Unshackle uses native helper | improved | Unshackle stronger; keep optional and explicit. Signed MSI packaging is deferred. |
...
- `src/background/settings/settings-store.ts` defaults `captureCredentialHeaders` to `false`, and `src/background/network/header-context.ts` can store `cookie` and `authorization` only when that flag is enabled. This keeps release defaults safer than broad reference-tool header capture while preserving an advanced opt-in path.
...
| P0 | Production HLS/DASH path audit | Unified + current source | `entrypoints/background.ts`, `src/background/jobs/download-controller.ts` | Native export remains available; HLS browser fallback now resolves routes before download, probes first media-segment bytes before mux.js, streams through offscreen sinks, keeps mux.js out of the background bundle, and refuses protected/unsupported/oversized routes early. |
| P1 | Broken-pipe/range recovery policy | live-stream `mget/plugins/error.js` | `src/core/download/segment-scheduler.ts`, HLS/DASH/direct runners | Implemented typed non-retry status handling, timeout tuning, and partial-range recovery in the scheduler; direct range downloads are tracked separately. Default `chrome.downloads`-based direct downloads are now tracked (`direct-download-tracker.ts`): an `interrupted` transition (e.g. SERVER_FORBIDDEN/NETWORK_FAILED) flips the job to `failed` instead of leaving the optimistic `completed` state, so mid-transfer failures surface. |
...
| P1 | Timeline/discontinuity handling | live-stream `index.js` | `src/core/hls/plan-hls-segments.ts`, `src/ui/queue/QueueItem.tsx` | Done: planner grouping plus a queue-level skip-ads/include-all toggle threaded through `DownloadSelection.discontinuityPolicy` and `SET_HLS_DISCONTINUITY_POLICY`. |
...
| P2 | Direct-to-disk/browser playable save mode | live-stream File System Access path | `src/core/export/*`, UI settings | Browser HLS fallback now has File System Access, OPFS, and bounded memory sinks for MP4 output; raw TS/staged segment downloads are refused, missing/unsafe segment probe evidence prevents mux.js, and mux.js failures require native FFmpeg instead of masquerading as successful MP4. |
...
| Preview | HLS preview in popup with native HLS fallback | `PlaylistPreview` | improved (`PreviewModal` plus queued asset pipeline) | Full preview uses the original HLS manifest while hover clips are generated as separate background assets. |
| Preview | hls.js preview when native HLS unsupported | `Hls.isSupported()` | present (lazy `import('hls.js')` + observable `playbackStrategy`) | `usePreviewPlayer` reports `native` vs `hls-js` and `PreviewModal` surfaces the active strategy (`data-testid="preview-playback-strategy"`); poster/hover extraction stays queued/deduped outside UI refresh loops with persisted asset metadata across background restarts. |
| Preview | Preview reload button | `PlaylistPreview` | present (reload icon in PreviewModal header) | Small UX improvement. |
| Preview | Preview duration callback from metadata or LEVEL_LOADED | `onDuration` | present (`onDurationResolved` → `SET_CANDIDATE_DURATION`) | `SidePanelApp` consumes the resolved duration and persists it via `candidateRegistry.setDuration`, so metadata updates flow back to the candidate. |
...
| Download | Generates MP4 unless subtitles selected, then MKV | add job epic | done | `resolveOutputContainer` now drives native export output: selected subtitles produce MKV names and `outputKind: mkv`; no-subtitle exports default to MP4. |
| Download | Stores subtitle text before mux/save | `storeSubtitleTextFactory`, save epic restorage | done | Native export fetches and stores selected subtitle text in `SubtitleStore` before mux, including sidecar filename metadata for failure recovery. |
| Download | Download job queue action after job creation | jobs slice and queue epic | present | Keep. |
| Download | Active download limit, 0 means unlimited | config and queue epic | partial | Unshackle has max concurrent; consider explicit unlimited semantics. `maxConcurrentDownloads` now reaches the queue at runtime via `setMaxConcurrent` (construction + live settings change); previously the queue ignored the setting and used a hardcoded 3. |
...
| Download | Ready/done job can save again | done | QueueItem overflow menu exposes `Save again`; SidePanelApp and runtime queue actions re-enqueue completed jobs with the prior selection. |
...
| Storage | Bucket metadata persisted separately | `bucketMeta` storage key | done | Scheduler `onFragmentStored` records each fresh fragment write; metadata persists per-bucket to chrome.storage.local. Matches puemos. |
| Storage | Track bytes written and stored chunks | `trackBucketUsage` | done | Production HLS writes increment bytes/chunk counts; resumed fragments excluded to avoid double-counting. |
| Storage | Serialize metadata updates per bucket | `metaUpdateQueues` | done | Per-bucket serialize() queue with durable write-through. |
| Storage | Rehydrate bucket object from metadata | `getBucket` | done | `bucketMetadataStore.rehydrate()` runs in background `rehydrateState()` on SW wakeup. |
| Storage | Measure bucket usage if metadata missing | `measureBucketUsage` | done | `GET_STORAGE_DIAGNOSTICS` consumes `estimateBucketUsage()` with stored-fragment fallback when metadata is absent. |
| Storage | Separate subtitles IndexedDB | `subtitles` DB | done | Background wires `createIndexedDbSubtitleStore()` (IndexedDB-backed, memory fallback). |
| Storage | Estimate subtitle byte usage | `estimateSubtitlesBytes` | done | Subtitle byte totals flow into `GET_STORAGE_DIAGNOSTICS` and render in footer/popup summary. |
| Storage | Browser quota estimate via `navigator.storage.estimate` | `getStorageEstimate` | done | Background diagnostics call `navigator.storage.estimate`; UI consumes the summary over runtime messaging. |
| Storage | Near-quota warning threshold at 90 percent or <=200MB free | storage stats use case | done | UI now consumes the canonical `storageQuotaLevel` (90%/<=200MB); side-panel and popup share the same policy and warning copy. |
| Storage | Cleanup all buckets and subtitle DB | `cleanup` | improved | Settings now exposes a cleanup action that removes orphaned browser fragment buckets and cached previous-session/thumbnail records while preserving active jobs, settings, and saved output files; full destructive reset still needs confirmation. |
...
| Storage | Storage summary in Settings and Downloads footer | UI | done | Popup "Storage & Output" shows a diagnostics-backed usage summary (used/quota + fragment/subtitle breakdown + warning); Downloads footer shares the same source. |
| Storage | Low storage banner component | `StorageBanner` | done | `StorageFooter` renders diagnostics warning copy (role="alert") and a breakdown line; popup summary surfaces the same warning. |
...
| Export | Subtitle mux to MKV with WebVTT | output `.mkv`, `video/x-matroska` | partial-core | `verifySubtitleTrack` exists; production native post-mux verification remains deferred. Sidecar fallback is now wired: the ffmpeg native path delivers fetched subtitle content as sidecar downloads on success (sidecar/both, and embed as a fallback since exportMedia carries no subtitle URLs to embed). |
| Export | `-shortest` for muxed outputs | ffmpeg args | partial-core | Helper arg support exists; production native helper parity is deferred. |
...
| Save | Ignore empty link gracefully | tests + DOM scanners | done | `isEmptyLink` is used in media/embed/iframe/selected-link scanning paths. |
...
| Naming | Subtitle filename with language/name fallback | `generate-subtitle-file-name.ts` | done | `deriveSubtitleFilename` is wired into subtitle pre-storage and emits `{videoName}.{language|trackName|und}.{format}`. |
...
| Regex classification | Regex rules can extract names, override extensions, blacklist, and test matches in the options UI (`js/init.js`, `js/options.js`). | done | UnifiedVideoDownloader has site heuristics; cat-catch exposes generic rules. | Regex rules are persisted, editable/importable/exportable in PopupApp, and applied by RequestJournal classification before candidate creation. |
...
| Duplicate handling | Optional duplicate URL and duplicate filename filtering, including preview-page duplicate filename cleanup (`js/init.js`, `js/preview.js`). | done | DuplicateBadge primitive plus MediaCard `duplicateCount`/`onDuplicateClick` props expose the affordance; SidePanelApp groups duplicate URL/title pairs for real candidates. |
| Badge and commands | Badge count, pause/enable, auto-download, clear, reboot, catch, m3u8, and deep-search keyboard commands (`js/background.js`, manifests). | done | `wxt.config.ts` declares `pause-all` (Ctrl+Shift+P), `clear-completed` (Ctrl+Shift+X), and `open-side-panel` (Ctrl+Shift+D); background command handlers implement all three. Safe commands only; advanced toggles remain settings-gated. `PAUSE_ALL_DOWNLOADS` now actually aborts the in-flight runs through the queue (previously it only flipped phase to `paused` while the download kept running and became unresumable); a `RESUME_ALL_DOWNLOADS` path revives paused jobs and re-drains. |
...
| Auto-download | Per-tab auto-download toggle starts downloads when new candidates arrive (`js/background.js`, `js/popup.js`). | partial/done | UnifiedVideoDownloader includes auto actions; puemos focuses manual HLS jobs. | Added guarded auto-download for advanced-mode direct, unprotected candidates with min-size and blacklist settings; broader per-tab controls remain future work. |
...
| Preview grid | `preview.html` lazily previews image/video/HLS resources, filters by type/regex, sorts by time/size/duration, paginates, tracks failed previews, and batch downloads/copies (`js/preview.js`). | done | Stronger preview ergonomics than live-stream-downloader and puemos. | Side-panel advanced tools mount `PreviewGrid` for real candidates with batch download/copy/remove/retry actions. |
...
| Options UI | Extensive settings for filters, regexes, blocklist, templates, player, Aria2, send-to-local, local protocol invocation, downloader, M3U8 parser, preview, side panel, custom CSS, MQTT, import/export/reset (`options.html`, `js/options.js`). | partial/done | cat-catch has the richest settings surface; puemos has cleaner typed state. | Capture rules, regexes, command templates, auto-download, Aria2, webhook, and external player profiles are now exposed; custom CSS/MQTT remain out of scope. |
| Settings import/export | Exports/imports base64 or JSON settings with timestamp and reset/clear tools (`js/options.js`). | present | More complete than live-stream-downloader. | Added versioned JSON import/export with `_exportedAt`, current-schema filtering, and secret/internal field redaction. |
| Copy/name templates | Supports replacement tags and transforms for title, URL, filename, headers, m3u8dl args, local invoke, and copy formats (`js/init.js`, `js/options.js`). | done | Richer than all prior references. | Safe copy/share templates are exposed via PopupApp custom command setting and queue command copy; cookie/auth/referer/origin values require advanced mode. |
...
| HLS manual ingest | `m3u8.html` accepts URL, raw manifest text, file input, raw TS URL lists, base URL overrides, `${range:start-end,pad}` expansion, and custom request headers (`js/m3u8.js`). | present | Added core `${range:start-end,pad}` expansion helper and side-panel HLS ingest for URLs, raw manifest text, file-loaded text, raw TS lists, and base URL overrides. | Keep request profiles safe; custom sensitive headers remain out of default scope. |
| HLS variant selection | Uses hls.js to parse levels, default to max bandwidth, list audio/subtitle tracks, and open selected variants (`js/m3u8.js`). | present | puemos has cleaner typed tests for audio/subtitle compatibility. | `defaultQualityPolicy` now threads through controller and HLS planning so highest/lowest policies auto-select without stale UI variant IDs. |
...
| Segment selection | Users can select/deselect arbitrary fragments, regex-filter segments, invert selection, pick index/time ranges, and choose discontinuity groups (`js/m3u8.js`, changelog 2.6.8). | present | Repair selector (failed indexes, index/time ranges, regex filters) is now wired through `REPAIR_HLS_SEGMENTS` + queue overflow-menu repair actions; discontinuity grouping exposed via the skip-ads toggle. | Done at the selector level; further per-fragment toggling stays future UX. |
...
| Transmux/merge | Uses mux.js for TS-to-MP4, optional audio-only, duration fixes, Blob merge, direct Chrome download, and online/iframe FFmpeg handoff (`js/m3u8.js`, `lib/mux.min.js`). | present | Unshackle's native FFmpeg path is stronger; puemos uses ffmpeg.wasm. | Native helper remains primary; browser HLS fallback uses real `mux.js` under a conservative size limit, surfaces post-fetch phases, and stops oversized browser-only blob exports with a clear native/direct-to-disk requirement. |
| Live HLS recording | Handles live playlists, fragment retry, record-mode UI, no-segment retry counts, and auto-close settings (`js/m3u8.js`, `js/init.js`). | improved | live-stream-downloader is also strong here. | No-new-segment retry telemetry now persists on the job and renders in the queue (live/idle, last sequence, idle retries); record-mode auto-close UI stays future work. |
...
| P1 | Add advanced capture-rule editor | settings, candidate classifier tests | Done with settings UI for extension/MIME/blacklist/min-size/size predicates, validation, import/export, and reset. |
...
| P1 | Add manual HLS ingest modes | side panel/parser route | Done for URL text, raw manifest text, file-loaded text, raw TS list conversion, and base URL override. |
| P1 | Add HLS segment repair controls | HLS job detail UI and tests | Done: `selectSegmentsForRepair()` is wired via `REPAIR_HLS_SEGMENTS` (index/time/regex) and queue overflow-menu repair actions; `JobSegmentStatus.durationSec` enables time-range repair. |
| P1 | Add HLS range expansion tests | HLS parser/planner fixtures | Done: `${range:start-end,pad}` expansion is wired into `createManualHlsIngestEvidence` so manual ingest accepts range templates. |
| P1 | Add live HLS retry telemetry | `src/core/hls/live-hls-telemetry.ts`, `src/core/hls/run-hls-job.ts` | Done: telemetry snapshots persist onto `DownloadJob.liveHlsTelemetry` and render in the queue item as a live/idle line. |
...
| Custom extensions | Options support newline-separated custom extensions (`customExtPref`, `customExtEntries`). | present | cat-catch has richer rule editing. | Custom extension capture rules now affect passive request journaling and candidate evidence. |
| Custom content types | Options support newline-separated custom content types (`customCtPref`, `customCtEntries`). | present | Valuable small operational feature. | Custom content-type capture rules now affect passive request journaling and candidate evidence. |
| Ignore filters | Users can ignore subtitles, direct files, manifests, small files below MB threshold, and blacklist URL/type/content-type/source substrings (`background.js`, `options.html`). | present | More focused than cat-catch's regex/size filters. | Glob blacklist, minimum-size, and size-predicate rules now gate passive request evidence before storage. |
...
| Storage lifecycle | Persists current `urlStorage`; on startup moves previous entries into `urlStorageRestore`, excluding private-window entries unless no-restore is enabled. | improved | Unshackle persists non-incognito candidates on tab close to `unshackle:previousDetections`, exposes them in the Previous Session view, defaults `previousSessionLimit` to 50, keeps stored thumbnails compact by preserving WebP data thumbnails while dropping bulky non-WebP frame data, cleans job storage when cancelled/removed jobs are cleared, and exposes a settings cleanup action for orphaned browser fragment buckets plus cached detection assets. | Matches recall semantics while giving users storage-volume control. |
...
| Popup list | Popup shows type, filename, size, source, timestamp, delete, filter, current/all/previous tabs, copy all, clear list, disable, options. | improved | Side panel exposes Current Tab / All Tabs / Previous Session sub-tabs, follows active browser-tab changes, hydrates all passive tab buckets for All Tabs, and shows a count summary, but intentionally omits filter controls for the smaller detected list. | |
...
| User command templates | Three custom commands with `%url%`, `%filename%`, `%useragent%`, `%referer%`, `%cookie%`, `%proxy%`, `%origin%`, `%tabtitle%`, `%timestamp%`; optional regex replace. | done | cat-catch has even richer templating. | PopupApp exposes one safe custom template profile; queue command copy renders it through the guarded template engine. |
...
| Settings import/export | Options export all settings except current URL stores/version/newline; import JSON and reset via background message (`options.js`). | present | Similar to cat-catch; cleaner than ViewTube. | Added versioned settings import/export with secret redaction and future-schema rejection. |
...
| P1 | Add custom extension/content-type rules | settings/capture classifier | Done with validated settings-backed capture-rule engine lists. |
...
| P1 | Add blacklist and minimum-size guards | settings/capture filters | Done with glob URL blacklist, minimum-size guard, and optional size predicate. |
...
| Skip ffmpeg option | `-f` flag saves raw `.ts` | `skip_ffmpeg=1` | not-scope | cat-catch has raw download mode | Unshackle now refuses raw HLS segment downloads in the product path; browser HLS either creates MP4 via mux.js or requires native FFmpeg. |
...
| P2 | Add playable HLS browser export | `src/core/export/*`, settings | Done for browser fallback: primary action says `Download`; raw TS and staged segment artifacts are no longer exposed as successful downloads. Missing manifest codec hints can proceed when segment bytes prove mux.js-safe MPEG-TS/H.264/AAC or MP3; unsafe hints, missing/unsafe segment byte probes, oversized streams, fMP4 assembly, or failed mux.js exports require native FFmpeg for playable output. Offscreen export sessions now start at first segment append, use keepalive pings during active exports, serialize concurrent segment completions before mux.js receives bytes, and write sequential fragment decode times from HLS durations or mux timing events so player duration matches the selected media rather than source PTS offsets. |
...
| P3 | Add aria2 external tool profile | integrations/settings | Done: `src/integrations/aria2-client.ts` JSON-RPC client + user-configurable `aria2Enabled/RpcUrl/Secret` settings; webhook + player-launcher dispatcher in `external-hub.ts`. |
...
| Segment download | Per-segment visual status (pending/success/error icons) | present | cat-catch has per-segment controls | HLS scheduler progress now feeds queue job segment state and renders a colored `SegmentGrid`. |
| Segment download | Click-to-retry individual errored segments | present | cat-catch has per-fragment retry | Segment grid click-to-retry is wired through runtime HLS segment retry actions. |
| Segment download | Auto-retry all errored segments on 2-second interval | present | puemos retries with backoff | Unshackle uses explicit retry-all-failed controls plus scheduler backoff instead of fixed polling. |
| Segment download | Range download (start/end segment selection) | present | cat-catch has richer segment selection | Queue segment range selection updates job selection and filters HLS scheduler work. |
| Segment download | Force download of partially completed segments | present | cat-catch has force-download existing buffer | Queue exposes export-selected-range action that requeues a partial HLS export. |
...
| Transmux | TS-to-MP4 via mux.js | present | puemos uses ffmpeg.wasm; Unshackle uses native helper plus offscreen mux.js fallback | Implemented as browser-only HLS MPEG-TS to MP4 fallback in the offscreen export host, with background-safe segment transfer, byte-level TS/PAT/PMT probing, sink selection, typed mux errors, route/sink/segment byte diagnostics, phase reporting, and a background-bundle regression. |
...
| Streaming download | WritableStream/TransformStream detection and fallback | present | Modern API usage | `detectStreamingWriteCapabilities()` now feeds browser HLS route selection for File System Access, OPFS, WritableStream, and bounded memory decisions before segment fetch. |
...
| Segment downloading | Simple concurrent XHR pool with per-segment retry and auto-retry interval. Unshackle has stronger scheduling plus queue segment visualization, click-to-retry, retry-all-failed, and range/partial export controls. |
...
| Streaming writes | StreamSaver.js solves memory exhaustion on large files but depends on a remote MITM page. Unshackle now detects safer File System Access/OPFS/WritableStream capability and keeps native helper as the strongest path. |
| Range/partial download | Start/end segment range selector and force-download-existing are now present in the HLS queue segment controls and runtime actions. |
...
| Priority | Item | Target landing zone | Notes |
|---:|---|---|---|
| P2 | Add per-segment status visualization in HLS job detail | `src/ui/queue/*`, HLS job detail component | Done: queue renders `SegmentGrid` from production HLS progress state. |
| P2 | Add force-export of partial HLS downloads | `src/core/download/*`, job actions | Done: selected range export requeues HLS with `segmentRange`. |
| P2 | Add segment range selection for HLS jobs | HLS job creation UI, segment scheduler | Done: grid range selection updates job selection and scheduler range filtering. |
| P2 | Add periodic auto-retry for errored segments | `src/core/download/segment-scheduler.ts`, settings | Done as explicit retry-all-failed plus existing scheduler backoff, avoiding fixed interval polling. |
| P3 | Consider mux.js as lightweight browser-only TS-to-MP4 fallback | `src/core/export/muxjs-transmuxer.ts`, `src/offscreen/export-host.ts`, settings/capabilities | Done: `mux.js` is loaded only in offscreen, MPEG-TS segments transfer through a JSON-safe command protocol after TS/PAT/PMT probing, missing manifest codec hints are accepted when byte probing proves mux.js-compatible streams, offscreen sessions start lazily and stay alive during active exports, each segment is transmuxed with a fresh sequential base decode time to avoid corrupted long-duration MP4 metadata, repeated init boxes are deduped, failures carry route/sink/segment/mux-code diagnostics without saving raw segment downloads, concurrent segment completions are serialized in plan order, and the MV3 background bundle is tested against mux/preload imports. |
| P3 | Add streaming write feature detection | `src/core/storage/*`, `src/core/export/*` | Done: detection now gates browser HLS route selection and OPFS/File System Access/memory sink choice before network work starts. |
...
7. Keep **mux.js as the bounded browser-only TS-to-MP4 fallback** when native helper is absent (P3, Apache-2.0 licensed); do not reuse `hls.js` playback internals for export.
