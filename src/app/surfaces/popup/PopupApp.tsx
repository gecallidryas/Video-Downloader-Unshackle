...
function SettingsSection({ title, badge, description, defaultOpen = true, children }: SettingsSectionProps) {
  return (
    <details className="settings-section" open={defaultOpen || undefined}>
      <summary>
        {title}
        {badge ? <span className="settings-section__badge">{badge}</span> : null}
      </summary>
    ...
    <>
      ...
      <SettingsSection title="Appearance">
        <label className="popup__row">
          ...
          <select aria-label="Theme" value={theme} onChange={(e) => setTheme(e.target.value as Parameters<typeof setTheme>[0])} className="popup__select">
            <option value="dark">Dark</option>
            <option value="light">Light</option>
          </select>
        </label>
        <label className="popup__row">
          <span className="popup__label">Language</span>
          <select
            id="ui-language"
      ...
      <SettingsSection title="Detection">
        <label className="popup__row">
          ...
          <input type="checkbox" role="checkbox" aria-label="Auto-detect" checked={autoDetect} onChange={toggleAutoDetect} className="popup__toggle" />
        </label>
        <label className="popup__row">
          <span className="popup__label">Enable Auto-Scan</span>
          <input type="checkbox" role="checkbox" aria-label="Enable Auto-Scan" checked={autoScanEnabled} onChange={(e) => setAutoScanEnabled(e.target.checked)} className="popup__toggle" />
        </label>
        <label className="popup__row">
          <span className="popup__label">Network capture</span>
          <input type="checkbox" role="checkbox" aria-label="Network capture" checked={networkCaptureEnabled} onChange={(e) => setNetworkCaptureEnabled(e.target.checked)} className="popup__toggle" />
        </label>
      </SettingsSection>
      ...
      <SettingsSection title="Downloads">
        <label className="popup__row">
          ...
          <select aria-label="Preferred quality" value={preferredQuality} onChange={(e) => setPreferredQuality(e.target.value as Parameters<typeof setPreferredQuality>[0])} className="popup__select">
        ...
        <label className="popup__row">
          ...
          <select aria-label="Max concurrent downloads" value={String(maxConcurrentDownloads)} onChange={(e) => setMaxConcurrentDownloads(Number(e.target.value))} className="popup__select">
            {[1, 2, 3, 5, 10].map((v) => <option key={v} value={v}>{v}</option>)}
          </select>
        </label>
        <label className="popup__row">
          <span className="popup__label">Segments per download</span>
          <select aria-label="Segments per download" value={String(maxConcurrentSegments)} onChange={(e) => setMaxConcurrentSegments(Number(e.target.value))} className="popup__select">
            {[3, 5, 10, 15].map((v) => <option key={v} value={v}>{v}</option>)}
          </select>
        </label>
        <label className="popup__row">
          <span className="popup__label">Output format</span>
          <select aria-label="Output format" value={defaultOutputFormat} onChange={(e) => setDefaultOutputFormat(e.target.value as Parameters<typeof setDefaultOutputFormat>[0])} className="popup__select">
        ...
        <label className="popup__row">
          ...
          <LanguagePicker id="preferred-audio-language" ariaLabel="Preferred audio language" value={preferredAudioLanguage} onChange={setPreferredAudioLanguage} />
        </label>
        <label className="popup__row popup__row--stack">
          <span className="popup__label">Filename template</span>
          <input aria-label="Filename template" value={namingTemplate} onChange={(e) => setNamingTemplate(e.target.value)} className="popup__input" />
        </label>
        <label className="popup__row">
          <span className="popup__label">Download notifications</span>
          <input type="checkbox" role="checkbox" aria-label="Notifications" checked={notifications} onChange={toggleNotifications} className="popup__toggle" />
        </label>
        <label className="popup__row">
          <span className="popup__label">Context menu</span>
          <input type="checkbox" role="checkbox" aria-label="Context menu" checked={enableContextMenu} onChange={toggleContextMenu} className="popup__toggle" />
        </label>
        <label className="popup__row">
          <span className="popup__label">Previous session items limit</span>
          <select aria-label="Previous session limit" value={String(previousSessionLimit)} onChange={(e) => setPreviousSessionLimit(Number(e.target.value))} className="popup__select">
            {[10, 25, 50, 100, 200, 0].map((v) => <option key={v} value={v}>{v === 0 ? 'Unlimited' : v}</option>)}
          </select>
        </label>
      </SettingsSection>

      {/* â”€â”€ Previews â”€â”€ */}
      <SettingsSection title="Previews">
        <label className="popup__row">
          <span className="popup__label">Preview mode</span>
          <select aria-label="Preview mode" value={previewMode} onChange={(e) => setPreviewMode(e.target.value as Parameters<typeof setPreviewMode>[0])} className="popup__select">
        ...
        <label className="popup__row">
          ...
          <select aria-label="Preview format" value={previewFormat} onChange={(e) => setPreviewFormat(e.target.value as Parameters<typeof setPreviewFormat>[0])} className="popup__select">
      ...
      <SettingsSection title="Storage & Output">
        <label className="popup__row popup__row--with-help">
          <span>
            ...
            <span className="popup__help">Streams supported exports into a user-selected folder instead of buffering in extension storage.</span>
          </span>
          <input type="checkbox" role="checkbox" aria-label="Use direct-to-disk when available" checked={useDirectToDisk} onChange={(event) => setUseDirectToDisk(event.target.checked)} className="popup__toggle" />
        </label>
        <label className="popup__row popup__row--with-help">
          <span>
            <span className="popup__label">Remember output folder</span>
            <span className="popup__help">Reuses the granted File System Access folder when the browser allows persisted handles.</span>
          </span>
          <input type="checkbox" role="checkbox" aria-label="Remember output folder" checked={rememberOutputFolder} onChange={(event) => setRememberOutputFolder(event.target.checked)} className="popup__toggle" />
        </label>
        <button type="button" className="popup__button" onClick={() => void chooseOutputFolder()}>Choose output folder</button>
        <label className="popup__row popup__row--with-help">
          <span>
            <span className="popup__label">Auto-delete fragments after save</span>
            <span className="popup__help">Removes temporary segment, subtitle, and metadata buckets after a completed download is saved.</span>
          </span>
      ...
      <SettingsSection title="Advanced">
        <label className="popup__row">
          ...
          <input type="checkbox" role="checkbox" aria-label="Advanced mode" checked={advancedMode} onChange={() => setAdvancedMode(!advancedMode)} className="popup__toggle" />
        </label>
        {advancedMode ? (
          <label className="popup__row popup__row--stack">
            <span className="popup__label">Custom command template</span>
            <textarea aria-label="Custom command template" value={customCommandTemplate} onChange={(event) => setCustomCommandTemplate(event.target.value)} className="popup__textarea" rows={2} />
          </label>
        ) : null}
      </SettingsSection>

      {/* â”€â”€ Browser Fallbacks (advanced only) â”€â”€ */}
      {advancedMode ? (
        <SettingsSection title="Browser Fallbacks" badge="Advanced" defaultOpen={false}>
          <label className="popup__row popup__row--with-help">
            <span>
              <span className="popup__label">Browser fallbacks</span>
              <span className="popup__help">Allows HLS/DASH saves and browser-generated WebM preview or trim paths.</span>
            </span>
            <input type="checkbox" role="checkbox" aria-label="Browser fallbacks" checked={enableBrowserFallbacks} onChange={(event) => setEnableBrowserFallbacks(event.target.checked)} className="popup__toggle" />
          </label>
          <label className="popup__row popup__row--with-help">
            <span>
              <span className="popup__label">mux.js HLS MP4 fallback</span>
              <span className="popup__help">Converts downloaded MPEG-TS HLS segments to MP4 in the browser when native export is unavailable.</span>
            </span>
            <input type="checkbox" role="checkbox" aria-label="mux.js HLS MP4 fallback" checked={browserTransmuxWithMuxJs} onChange={(event) => setBrowserTransmuxWithMuxJs(event.target.checked)} className="popup__toggle" disabled={!enableBrowserFallbacks} />
          </label>
          <label className="popup__row">
            <span className="popup__label">mux.js size limit (MB)</span>
            <input aria-label="mux.js size limit" type="number" min={1} value={Math.round(browserTransmuxMaxBytes / 1024 / 1024)} onChange={(event) => setBrowserTransmuxMaxBytes(Number(event.target.value) * 1024 * 1024)} className="popup__input" disabled={!enableBrowserFallbacks || !browserTransmuxWithMuxJs} />
          </label>
        </SettingsSection>
      ) : null}

      {/* â”€â”€ Capture Rules (advanced only) â”€â”€ */}
      {advancedMode ? (
        <SettingsSection title="Capture Rules" badge="Advanced" defaultOpen={false}>
          <div className="capture-rules">
            <label className="popup__row popup__row--stack">
              <span className="popup__label">Custom extensions</span>
              <textarea aria-label="Custom extensions" value={customExtensionsDraft} onChange={(event) => { setCustomExtensionsDraft(event.target.value); updateCaptureRules({ customExtensions: linesToList(event.target.value) }); }} className="popup__textarea" rows={2} />
            </label>
            <label className="popup__row popup__row--stack">
              <span className="popup__label">Custom content types</span>
              <textarea aria-label="Custom content types" value={customContentTypesDraft} onChange={(event) => { setCustomContentTypesDraft(event.target.value); updateCaptureRules({ customContentTypes: linesToList(event.target.value) }); }} className="popup__textarea" rows={2} />
            </label>
            <label className="popup__row popup__row--stack">
              <span className="popup__label">URL blacklist</span>
              <textarea aria-label="URL blacklist" value={urlBlacklistDraft} onChange={(event) => { setUrlBlacklistDraft(event.target.value); updateCaptureRules({ urlBlacklist: linesToList(event.target.value) }); }} className="popup__textarea" rows={2} />
            </label>
            <label className="popup__row popup__row--stack">
              <span className="popup__label">Minimum size bytes</span>
              <input aria-label="Minimum size bytes" type="number" min={0} value={captureRuleMinSizeBytes} onChange={(event) => updateCaptureRules({ minSizeBytes: Number(event.target.value) })} className="popup__input" />
            </label>
            <label className="popup__row popup__row--stack">
              <span className="popup__label">Size predicate</span>
              <input aria-label="Size predicate" value={sizePredicateDraft} onChange={(event) => { setSizePredicateDraft(event.target.value); updateCaptureRules({ sizePredicate: event.target.value }); }} className="popup__input" />
            </label>
            <label className="popup__row popup__row--stack">
              <span className="popup__label">Regex classification rules</span>
              <textarea aria-label="Regex classification rules" value={regexRulesDraft} onChange={(event) => { setRegexRulesDraft(event.target.value); try { const parsed = JSON.parse(event.target.value) as RegexRule[]; updateCaptureRules({ regexRules: parsed }); } catch (error) { setCaptureRulesError(error instanceof Error ? error.message : 'Invalid regex rules JSON'); } }} className="popup__textarea" rows={3} />
            </label>
            <label className="popup__row popup__row--stack">
              <span className="popup__label">Capture rules JSON</span>
              <textarea aria-label="Capture rules JSON" value={captureRulesJson} onChange={(event) => setCaptureRulesJson(event.target.value)} className="popup__textarea" rows={3} />
            </label>
            {captureRulesError ? <p className="capture-rules__error">{captureRulesError}</p> : null}
            <div className="capture-rules__actions">
              <button type="button" onClick={exportCaptureRules} className="capture-rules__button">Export capture rules</button>
              <button type="button" onClick={importCaptureRules} className="capture-rules__button">Import capture rules</button>
              <button type="button" onClick={() => { resetCaptureRules(); setCustomExtensionsDraft(''); setCustomContentTypesDraft(''); setUrlBlacklistDraft(''); setSizePredicateDraft(''); setRegexRulesDraft('[]'); setCaptureRulesError(null); }} className="capture-rules__button">Reset capture rules</button>
            </div>
          </div>
        </SettingsSection>
      ) : null}

      {/* â”€â”€ Automation & Integrations (advanced only) â”€â”€ */}
      {advancedMode ? (
        <SettingsSection title="Automation & Integrations" badge="Advanced" defaultOpen={false}>
          <label className="popup__row">
            <span className="popup__label">Auto-download safe direct media</span>
            <input aria-label="Auto-download safe direct media" type="checkbox" role="checkbox" checked={autoDownloadEnabled} onChange={(event) => setAutoDownloadSettings({ enabled: event.target.checked })} className="popup__toggle" />
          </label>
          <label className="popup__row popup__row--stack">
            <span className="popup__label">Auto-download minimum size</span>
            <input aria-label="Auto-download minimum size" type="number" min={0} value={autoDownloadMinSize} onChange={(event) => setAutoDownloadSettings({ minSize: Number(event.target.value) })} className="popup__input" />
          </label>
          <label className="popup__row popup__row--stack">
            <span className="popup__label">Auto-download blacklist</span>
            <textarea aria-label="Auto-download blacklist" value={autoDownloadBlacklistDraft} onChange={(event) => { setAutoDownloadBlacklistDraft(event.target.value); setAutoDownloadSettings({ blacklist: linesToList(event.target.value) }); }} className="popup__textarea" rows={2} />
          </label>
          <label className="popup__row">
            <span className="popup__label">Enable Aria2</span>
            <input aria-label="Enable Aria2" type="checkbox" role="checkbox" checked={aria2Enabled} onChange={(event) => setAria2Settings({ enabled: event.target.checked })} className="popup__toggle" />
          </label>
          <label className="popup__row popup__row--stack">
            <span className="popup__label">Aria2 RPC URL</span>
            <input aria-label="Aria2 RPC URL" value={aria2RpcUrl} onChange={(event) => setAria2Settings({ rpcUrl: event.target.value })} className="popup__input" />
          </label>
          <label className="popup__row popup__row--stack">
            <span className="popup__label">Aria2 secret</span>
            <input aria-label="Aria2 secret" type="password" value={aria2Secret} onChange={(event) => setAria2Settings({ secret: event.target.value })} className="popup__input" />
          </label>
          <label className="popup__row">
            <span className="popup__label">Enable webhook</span>
            <input aria-label="Enable webhook" type="checkbox" role="checkbox" checked={webhookEnabled} onChange={(event) => setWebhookSettings({ enabled: event.target.checked })} className="popup__toggle" />
          </label>
          <label className="popup__row popup__row--stack">
            <span className="popup__label">Webhook URL</span>
            <input aria-label="Webhook URL" value={webhookUrl} onChange={(event) => setWebhookSettings({ url: event.target.value })} className="popup__input" />
          </label>
          <label className="popup__row popup__row--stack">
            <span className="popup__label">External player profiles JSON</span>
            <textarea aria-label="External player profiles JSON" value={externalPlayerProfilesDraft} onChange={(event) => { setExternalPlayerProfilesDraft(event.target.value); try { const parsed = JSON.parse(event.target.value) as ExternalPlayerProfile[]; setExternalPlayerProfiles(parsed); } catch { /* Keep the draft editable until valid JSON is supplied. */ } }} className="popup__textarea" rows={3} />
          </label>
        </SettingsSection>
      ) : null}
    </>
        ...
        <li key={job.id} style={{ borderBottom: '1px solid var(--outline-variant, #2a2a2a)' }}>
          ...
          >
            ...
            <span style={{ marginLeft: 8, opacity: 0.7 }}>
              {job.status} Â· {Math.round(job.progressPct)}%
            </span>
          </button>
        </li>
      ))}
