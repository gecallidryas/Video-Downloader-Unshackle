...
function renderPowerShellBody(values: {
  ...
  return [
    ...
    "    if ($Response.BaseResponse.ResponseUri.AbsoluteUri -match '/tag/([^/?#]+)') { return $Matches[1] }",
    '  } catch {',
    '    $Location = $_.Exception.Response.Headers.Location',
    "    if ($Location -and ($Location.ToString() -match '/tag/([^/?#]+)')) { return $Matches[1] }",
    '  }',
    "  throw ('Could not resolve the latest release tag from ' + $ReleaseBaseUrl + '/latest.')",
    '}',
    '',
    ...
    '  Invoke-WebRequest -Uri ($DownloadBase + $ZipFileName) -OutFile $ZipPath',
    '  Invoke-WebRequest -Uri ($DownloadBase + $ChecksumFileName) -OutFile $ChecksumPath',
    ...
    "$DownloadDir = Join-Path ([System.IO.Path]::GetTempPath()) ('unshackle-native-helper-download-' + ([guid]::NewGuid().ToString('N')))",
